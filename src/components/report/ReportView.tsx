...
function SecLabel({ txt, section, dark }: { txt: string; section?: string; dark?: boolean }) {
  ...
  return (
    <div style={{ fontFamily: MONO, fontSize: "11px", color: c, letterSpacing: "2px", textTransform: "uppercase", display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
      <span style={{ display: "block", width: "28px", height: "1px", background: bc }} />
      {txt}{section ? ` · ${section}` : ""}
    </div>
  );
}
...
function Bar({ pct, color, h = 6 }: { pct: number; color?: string; h?: number }) {
  return (
    <div style={{ height: `${h}px`, width: "100%", background: G200, borderRadius: `${h / 2}px`, overflow: "hidden" }}>
      <div style={{ height: "100%", width: `${Math.min(100, Math.max(0, pct))}%`, background: color ?? BLK, borderRadius: `${h / 2}px`, transition: "width .4s" }} />
    </div>
  );
}
...
function DotBadge({ status }: { status: "up" | "dn" | "fl" | "wn" }) {
  const map = { up: { bg: "#DCFCE7", c: "#16A34A", t: "▲ 양호" }, dn: { bg: "#FEE2E2", c: "#DC2626", t: "▼ 주의" }, fl: { bg: "#F3F4F6", c: "#6B7280", t: "→ 보통" }, wn: { bg: "#FEF3C7", c: "#D97706", t: "⚠ 둔화" } };
  const s = map[status];
  return (
    ...
    <section style={{ background: BLK, paddingBottom: 0 }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "64px 48px 0" }}>
        ...
        {(() => {
          ...
          const items = [
            { v: fmt(scorecard?.score_breakdown?.growth_velocity?.score), l: "성장 속도 점수", d: breakdown?.growth_velocity?.grade ?? "" },
            { v: fmt(scorecard?.score_breakdown?.upload_regularity?.score), l: "업로드 규칙성", d: breakdown?.upload_regularity?.grade ?? "" },
            { v: `${scorecard?.score_breakdown?.viral_potential?.score ?? "-"}`, l: "바이럴 잠재력", d: breakdown?.viral_potential?.grade ?? "" },
            { v: `${scorecard?.score_breakdown?.engagement_quality?.score ?? "-"}`, l: "참여 품질", d: breakdown?.engagement_quality?.grade ?? "" },
            { v: `${scorecard?.score_breakdown?.niche_authority?.score ?? "-"}`, l: "니치 권위", d: breakdown?.niche_authority?.grade ?? "" },
            { v: `${scorecard?.score_breakdown?.content_consistency?.score ?? "-"}`, l: "콘텐츠 일관성", d: breakdown?.content_consistency?.grade ?? "" },
          ];
          return (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(6,1fr)", gap: "1px", background: DARK3, border: `1px solid ${DARK3}`, borderTop: "none", marginBottom: "48px" }}>
              {items.map((k, i) => (
                <div key={i} style={{ background: "#1E1E1E", padding: "20px 16px" }}>
                  <div style={{ fontSize: "18px", fontWeight: 800, color: "#fff", fontFamily: MONO, letterSpacing: "-.5px", lineHeight: 1, marginBottom: "10px" }}>{k.v}</div>
        ...
        {(scorecard?.strengths?.length || scorecard?.weaknesses?.length) ? (
          <div style={{ borderTop: `1px solid ${DARK3}`, paddingBottom: "48px", paddingTop: "36px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2px", background: DARK3, border: `1px solid ${DARK3}`, marginBottom: "0" }}>
        ...
        {(data.top10_videos ?? []).length > 0 && (
          <div style={{ marginTop: "40px" }}>
            <div style={{ fontFamily: MONO, fontSize: "11px", color: G400, letterSpacing: "2px", textTransform: "uppercase", marginBottom: "16px", display: "flex", alignItems: "center", gap: "10px" }}>
              <span style={{ width: "28px", height: "1px", background: G400, display: "block" }} />TOP 10 영상
            </div>
            <div style={{ border: `1px solid ${G200}` }}>
              {(data.top10_videos ?? []).map((v, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: "16px", padding: "12px 20px", borderBottom: i < (data.top10_videos ?? []).length - 1 ? `1px solid ${G200}` : "none", background: i % 2 === 0 ? "#fff" : G100 }}>
                  <span style={{ fontFamily: MONO, fontSize: "12px", fontWeight: 700, color: ORANGE, minWidth: "24px", flexShrink: 0 }}>{v.rank ?? i + 1}</span>
                  <span style={{ flex: 1, fontSize: "14px", color: BLK, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{v.title ?? "-"}</span>
                  <span style={{ fontFamily: MONO, fontSize: "13px", fontWeight: 700, color: G600, flexShrink: 0 }}>{fmt(v.views)}</span>
                  <span style={{ fontFamily: MONO, fontSize: "11px", color: G400, flexShrink: 0 }}>{v.date ?? ""}</span>
                </div>
              ))}
            </div>
          </div>
...
function DataSignalsSection({ data }: { data: ManusReportJson["section3_data_signals"] }) {
  ...
  const group1 = [
    title?.avg_title_length != null && { n: "01", t: "평균 제목 길이", v: `${title.avg_title_length}자`, dot: "g" as const },
    title?.optimal_title_length && { n: "02", t: "최적 제목 길이", v: title.optimal_title_length, dot: "g" as const },
    title?.hashtag_usage?.avg_tags != null && { n: "03", t: "평균 태그 수", v: `${title.hashtag_usage.avg_tags}개`, dot: "b" as const },
    ...(title?.effective_structures ?? []).slice(0, 3).map((s, i) => ({ n: `0${4 + i}`, t: "효과적 제목 구조", v: s.slice(0, 40), dot: "g" as const })),
    ...(kw?.high_ctr_keywords ?? []).slice(0, 3).map((k, i) => ({ n: `0${7 + i}`, t: "고 CTR 키워드", v: `#${k}`, dot: "g" as const })),
  ].filter(Boolean).slice(0, 10);

  const group2 = high.slice(0, 10).map((p, i) => ({
    n: String(i + 1).padStart(2, "0"),
    t: p.pattern ?? "-",
    v: p.avg_views != null ? `평균 ${fmt(p.avg_views)}회` : (p.description ?? "").slice(0, 30),
    dot: "g" as const,
  }));

  const group3 = [
    ...low.map((p, i) => ({ n: String(i + 1).padStart(2, "0"), t: p.pattern ?? "-", v: (p.description ?? "").slice(0, 30), dot: "r" as const })),
    ...(kw?.topic_performance ? Object.entries(kw.topic_performance).map(([topic, perf], i) => ({
      n: String(low.length + i + 1).padStart(2, "0"),
      t: `주제: ${topic}`,
      v: perf?.avg_views != null ? `평균 ${fmt(perf.avg_views)}회 · ${perf.share_pct ?? 0}%` : "",
      dot: "b" as const,
    })) : []),
  ].slice(0, 10);
  ...
  return (
    <section style={{ background: DARK, padding: "72px 0" }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "0 48px" }}>
        ...
        <h2 style={{ fontSize: "30px", fontWeight: 900, letterSpacing: "-1px", lineHeight: 1.2, marginBottom: "8px", color: "#fff", fontFamily: SANS }}>데이터 시그널</h2>
...
function ChannelPatternsSection({ data }: { data: ManusReportJson["section4_channel_patterns"] }) {
  ...
  if (upload) patterns.push({
    n: "01", name: "업로드 패턴 (주기·요일·시간대)",
    char: [upload.avg_upload_interval_days != null && `평균 ${upload.avg_upload_interval_days}일 간격`, upload.recent_30d_uploads != null && `최근 30일 ${upload.recent_30d_uploads}건`, upload.upload_consistency].filter(Boolean).join(" · "),
    interp: upload.optimal_upload_frequency ? `최적 업로드 빈도: ${upload.optimal_upload_frequency}` : "정기 업로드 루틴 확립 권장",
  });
  if (thumb?.effective_title_formulas?.length) patterns.push({
    n: "02", name: "제목/썸네일 전략 패턴",
    char: (thumb.effective_title_formulas ?? []).slice(0, 2).join(" · "),
    interp: (thumb.effective_thumbnail_elements ?? []).slice(0, 2).join(", ") || "썸네일 일관성 유지 권장",
  });
  if (aud) patterns.push({
    n: "03", name: "시청자 반응 패턴 (좋아요 vs 댓글)",
    char: [aud.viral_trigger && `바이럴: ${aud.viral_trigger.slice(0,40)}`, aud.comment_driver && `댓글: ${aud.comment_driver.slice(0,40)}`].filter(Boolean).join(" · "),
    interp: aud.engagement_peak_content ?? "참여도 높은 콘텐츠 분석 중",
  });
  if (phases.length) {
    const p = phases[phases.length - 1];
    patterns.push({
      n: "04", name: "콘텐츠 진화 타임라인",
      char: phases.map((ph, i) => `${i+1}단계: ${ph?.theme ?? "-"}`).join(" → "),
      interp: p?.description ?? "채널 콘텐츠 방향이 수렴 중",
    });
  }
  ...
  return (
    <section style={{ padding: "72px 0", background: "#fff" }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "0 48px" }}>
        ...
        <h2 style={{ fontSize: "30px", fontWeight: 900, letterSpacing: "-1px", lineHeight: 1.2, marginBottom: "8px", fontFamily: SANS }}>채널 운영 패턴</h2>
        <p style={{ fontSize: "16px", color: G600, marginBottom: "40px" }}>발견된 패턴과 그 의미를 분석합니다.</p>
  ...
  return (
    <section style={{ background: "#161616", padding: "72px 0" }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "0 48px" }}>
        ...
        {/* strengths / weaknesses */}
        {((data.content_pillars ?? []).length > 0 || data.creator_persona || data.target_audience) && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2px", background: DARK3, border: `1px solid ${DARK3}`, marginBottom: "28px" }}>
            {data.creator_persona && (
              <div style={{ background: DARK2, padding: "28px" }}>
                ...
                {[["캐릭터", data.creator_persona.character], ["스토리텔링", data.creator_persona.storytelling_style], ["관계", data.creator_persona.relationship_with_audience]].filter(([, v]) => v).map(([k, v], i) => (
            ...
            {data.target_audience && (
              <div style={{ background: DARK2, padding: "28px" }}>
                ...
                {[["1차 타겟", data.target_audience.primary], ["2차 타겟", data.target_audience.secondary], ["3차 타겟", data.target_audience.tertiary]].filter(([, v]) => v).map(([k, v], i) => (
                  <div key={i} style={{ marginBottom: "14px" }}>
                    <div style={{ fontFamily: MONO, fontSize: "11px", color: "#888", marginBottom: "3px" }}>{k}</div>
                    <div style={{ fontSize: "14px", color: "#F0F0F0", lineHeight: 1.7 }}>{v}</div>
                  </div>
...
function ActionPlanSection({ data }: { data: ManusReportJson["section7_action_plan"] }) {
  ...
  function TaskRows({ tasks, label, color }: { tasks: Task[]; label: string; color: string }) {
    if (!tasks.length) return null;
    return (
      <div style={{ marginBottom: "32px" }}>
        <div style={{ fontFamily: MONO, fontSize: "11px", color, letterSpacing: "2px", textTransform: "uppercase", marginBottom: "16px", display: "flex", alignItems: "center", gap: "10px" }}>
          <span style={{ width: "28px", height: "1px", background: color, display: "block" }} />{label}
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {tasks.map((t, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: "16px", padding: "18px 22px", border: `1px solid ${G200}`, borderLeft: `3px solid ${t.priority?.toUpperCase() === "긴급" || t.priority?.toUpperCase() === "URGENT" ? "#DC2626" : t.priority?.toUpperCase() === "높음" || t.priority?.toUpperCase() === "HIGH" ? "#D97706" : "#1D4ED8"}` }}>
  ...
  return (
    <section style={{ padding: "72px 0", background: "#fff" }}>
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "0 48px" }}>
        ...
        <p style={{ fontSize: "16px", color: G600, marginBottom: "40px" }}>우선순위별 액션 아이템과 목표 수치를 확인하세요.</p>

        <TaskRows tasks={data.immediate_actions?.tasks ?? []} label={`즉시 실행 · ${data.immediate_actions?.timeframe ?? "1주 이내"}`} color="#DC2626" />
        <TaskRows tasks={data.short_term_plan?.tasks ?? []} label={`단기 계획 · ${data.short_term_plan?.timeframe ?? "2~3주"}`} color="#D97706" />
        <TaskRows tasks={data.long_term_plan?.tasks ?? []} label={`장기 계획 · ${data.long_term_plan?.timeframe ?? "4주 이후"}`} color={G400} />

        {/* risk management */}
        {(data.risk_management ?? []).length > 0 && (
          <div style={{ border: `1px solid #F59E0B`, background: "#FFFBEB", padding: "18px 22px", display: "flex", gap: "14px", alignItems: "flex-start", marginBottom: "32px" }}>
            <div style={{ fontSize: "18px", flexShrink: 0, paddingTop: "1px" }}>⚠</div>
        ...
        {kpi && (
          <>
            <div style={{ fontFamily: MONO, fontSize: "11px", color: G400, letterSpacing: "1px", textTransform: "uppercase", marginBottom: "14px", display: "flex", alignItems: "center", gap: "10px" }}>
              <span style={{ width: "28px", height: "1px", background: G400, display: "block" }} />KPI 목표 달성 예측
            </div>
                ...
                return (
                  <div key={key} style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    ...
                    <div style={{ flex: 1, height: "8px", background: G200, borderRadius: "4px", overflow: "hidden" }}>
                      <div style={{ width: `${Math.min(100, (key === "1_month" ? 25 : key === "3_months" ? 50 : key === "6_months" ? 75 : 100))}%`, height: "100%", background: "#16A34A", borderRadius: "4px" }} />
                    </div>
            ...
            {/* goals grid */}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "1px", background: G200, border: `1px solid ${G200}` }}>
              {(["1_month","3_months","6_months","12_months"] as const).filter(k => kpi[k]).map((key) => {
                const row = kpi[key]!;
                ...
                return (
                  <div key={key} style={{ background: "#fff", padding: "22px 18px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                      <span style={{ fontSize: "16px", fontWeight: 700, fontFamily: MONO, color: G400 }}>
                        {fmt(row.subscribers)}
                      </span>
                      <span style={{ fontSize: "13px", color: G400 }}>명</span>
                    </div>
                  </div>
                );
              })}
...
export default function ReportView({ report, generatedAt }: Props) {
  const date = new Date(generatedAt).toLocaleDateString("ko-KR", { year: "numeric", month: "2-digit" }).replace(". ", ".").replace(".", "년 ").replace(".", "월");
  ...
  return (
    <>
      ...
      <div className="report-root">
        {/* NAV */}
        <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(255,255,255,.96)", backdropFilter: "blur(12px)", borderBottom: `1px solid ${G200}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 48px", height: "58px" }}>
