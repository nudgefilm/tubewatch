...
/** GET — channel_dna_report 조회. 없으면 pending:true 반환 (생성은 메인 분석 route에서 처리). */
export async function GET(req: NextRequest) {
  try {
    ...
    if (mod.status === "pending") {
      ...
      if (startedAt === null) {
        console.warn("[onepager-api] timeout-fallback: started_at missing", { snapshot_id: snap.id, module_key: "channel_dna_report" });
