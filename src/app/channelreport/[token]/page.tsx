...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${name} 채널 전략 리포트 | Channel Report`,
    description: `${name} 채널의 Channel Report 월간 전략 분석 리포트입니다.`,
  };
}

export default async function ChannelReportTokenPage({ params }: Props) {
  const { token } = await params;
