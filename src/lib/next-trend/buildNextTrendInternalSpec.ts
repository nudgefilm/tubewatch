...
export function buildNextTrendInternalSpec(
  ...
  const actions: NextTrendActionsVm = {
    ...
    videoPlanDraft: isShorts ? [
      ...
      `**결론 (45~60초)** — "이게 핵심입니다" 한 줄 요약 + 구독·좋아요 유도.`,
    ...
    scriptOutline: (() => {
      ...
      if (isShorts) {
        return [
          ...
          `**③ 반전·결론** (45~60초) — "이게 전부입니다" 한 줄 요약 + 구독·좋아요 유도.`,
          `  → 권장 길이: 60초 이하 (Shorts)`,
        ].join("\n");
      }
    ...
    contentPlan: [
      ...
      `업로드 후 **48시간** 안에 썸네일 클릭률(CTR)을 확인하세요. 4% 미만이면 썸네일부터 교체하세요.`,
    ...
    expectedReaction: [
      targetAudience[0]
        ...
        : `기존 구독자 중심으로 실용적 정보를 원하는 시청자층이 주요 대상입니다.`,
      `업로드 후 **24시간 안에 썸네일 클릭률(CTR)**과 **시청 유지율**을 확인하세요. 채널 평균 이상이면 후속편을 바로 기획하세요.`,
