...
const insight =
  "현재 채널은 콘텐츠 구조 점수는 높지만 SEO 최적화와 시청자 반응에서 개선 여지가 큽니다. 업로드 빈도를 높이고 제목/설명/태그 최적화에 집중하면 더 빠른 성장이 가능합니다."

type Status = "높음" | "보통" | "낮음"
...
type ComparisonCard = (typeof comparisonCards)[number]

function TopSummaryCard(): JSX.Element {
  return (
...
function ComparisonScoreCards(): JSX.Element {
  return (
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      {comparisonCards.map((card: ComparisonCard) => {
          ...
          <Card key={card.id} className="hover-lift">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                ...
                <Badge variant="outline" className={getStatusBadgeStyle(card.status as Status)}>
                  {card.status}
                </Badge>
              </div>
...
function BenchmarkChartCard(): JSX.Element {
  const maxScore = 100
  return (
    <Card>
      <CardHeader>
...
function StrengthWeaknessSection(): JSX.Element {
  return (
...
function BenchmarkEmptyState(): JSX.Element {
  return (
